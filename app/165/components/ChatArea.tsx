...
type Props = {
  ...
  /** Extra px subtracted from 100dvh for container height. Default 240. */
  heightOffset?: number;
};
...
export default function ChatArea({
  ...
  // Load messages whenever convId changes
  useEffect(() => {
    setLoadingMsgs(true);
    setMessages([]);
    setReactions([]);
  ...
  return (
    <div style={{ flex: 1, display: "flex", height: `calc(100dvh - ${heightOffset}px)`, minWidth: 0 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
                  ...
                  <div style={{ position: "relative", maxWidth: "75%" }}>
                    ...
                    <div style={{ backgroundColor: isMine ? myBubble : theirBubble, borderRadius: 16, borderTopRightRadius: isMine ? 4 : 16, borderTopLeftRadius: isMine ? 16 : 4, padding: isMedia && !hasText ? "4px" : "8px 14px", overflow: "hidden" }}>
                      {isMedia && (
                        <div style={{ marginBottom: hasText ? 6 : 0 }}>
                          {isVideo ? (
                            <video src={msg.image_url!} controls style={{ maxWidth: 240, maxHeight: 200, borderRadius: 10, display: "block" }} />
                          ) : (
                            <img src={msg.image_url!} alt="media" style={{ maxWidth: 240, maxHeight: 280, borderRadius: 10, objectFit: "cover", display: "block", cursor: "zoom-in" }} onClick={(e) => { e.stopPropagation(); setLightboxUrl(msg.image_url!); }} />
                          )}
                        </div>
                      )}
                      {hasText && <p className="text-sm" style={{ wordBreak: "break-word" }}>{msg.content}</p>}
                    ...
                    {showEmojiFor === msg.id && (
                      <div style={{ position: "absolute", bottom: "100%", ...(isMine ? { right: 0 } : { left: 0 }), marginBottom: 4, backgroundColor: isDark ? "#1a1a1a" : "#fff", border: `1px solid ${borderSubtle}`, borderRadius: 12, padding: "6px 8px", display: "flex", flexWrap: "wrap", gap: 2, zIndex: 20, boxShadow: "0 4px 12px rgba(0,0,0,0.15)", maxWidth: 220 }}>
                        {EMOJI_LIST.map((e) => (
                          <button key={e} onClick={(ev) => { ev.stopPropagation(); handleReaction(msg.id, e); }} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", padding: "4px 2px", borderRadius: 6, lineHeight: 1, minWidth: 32 }}
        ...
        {(replyTo || editingMsg) && (
          <div style={{ padding: "8px 14px", borderTop: `1px solid ${borderSubtle}`, backgroundColor: bgSubtle, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={() => { setReplyTo(null); setEditingMsg(null); setInputValue(""); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, lineHeight: 0, color: fgMuted }}><X size={14} /></button>
          </div>
        )}
        ...
        <div style={{ padding: "10px 12px", paddingBottom: "calc(10px + env(safe-area-inset-bottom))", borderTop: `1px solid ${borderSubtle}`, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: uploadingFile ? "default" : "pointer", padding: 8, lineHeight: 0, color: fgMuted, opacity: uploadingFile ? 0.3 : 0.6, flexShrink: 0, minWidth: 36, minHeight: 44, display: "flex", alignItems: "center", justifyContent: "center" }}
          >
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: inputValue.trim() || pendingMedia ? "pointer" : "default", padding: 8, lineHeight: 0, color: inputValue.trim() || pendingMedia ? fg : fgMuted, opacity: inputValue.trim() || pendingMedia ? 1 : 0.4, transition: "opacity 0.15s", minWidth: 36, minHeight: 44, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
          >
      ...
      {showGroupPanel && isGroup && (
        <div style={{ width: 210, borderLeft: `1px solid ${borderSubtle}`, display: "flex", flexDirection: "column", flexShrink: 0, backgroundColor: isDark ? "rgba(0,0,0,0.3)" : "rgba(255,255,255,0.5)" }}>
